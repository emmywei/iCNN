...
# Cat/dog Still U-Net denoiser with an EfficientNet reader.
###############################################################
...
PT.DisplaySamples2(
    train_data,
    noisy_train,
    file_name='NoisyCatDogSamples.png',
    indices=train_indices
)


###############################################################
# Still U-Net denoiser
###############################################################
...
class Denoiser(nn.Module):
    ...
    def circ_max_pool(self, x):
        # LibShiftInvar expects channel-last tensors.
        x = x.permute(0, 2, 3, 1).contiguous()
        x = libinv.CircMaxPool2D(
            x,
            self.relu_like_act
        )
    ...
    def forward(self, x):
        ...
        e1 = self.enc1(x)
        p1 = self.circ_max_pool(e1)

        e2 = self.enc2(p1)
        p2 = self.circ_max_pool(e2)
...
history_loss, history_val_loss, denoiser_epoch_times = train_model(
    ...
    block_name='CatDogUNetDenoiser training'
)

torch.save(
    denoiser.state_dict(),
    'CatDogUNetDenoiser.pt'
)
...
PT.DisplaySamples3(
    ...
    noisy_test,
    denoised_test,
    file_name='CatDogDenoiserSamples.png',
...
with open('CatDogLossStill.txt', 'w') as file:
    for loss in denoiser_loss_list:
        file.write(str(loss) + '\n')

with open('CatDogValLossStill.txt', 'w') as file:
    for loss in denoiser_val_loss_list:
        file.write(str(loss) + '\n')

with open('CatDogEpochTimesStill.txt', 'w') as file:
...
plt.grid()
plt.savefig('CatDogDenoiserLosses.png')
plt.show()
...
if run_shift_analysis:
    ...
    with open('CatDogShiftTimesStill.txt', 'w') as file:
...
torch.save(
    reader.state_dict(),
    'CatDogCleanTrainedEfficientNetReader.pt'
)
...
with open('CatDogCleanReaderLoss.txt', 'w') as file:
    for loss in reader_loss:
        file.write(str(loss) + '\n')

with open('CatDogCleanReaderValLoss.txt', 'w') as file:
    for loss in reader_val_loss:
        file.write(str(loss) + '\n')

with open('CatDogCleanReaderEpochTimes.txt', 'w') as file:
...
with open('CatDogCleanReaderAccuracies.txt', 'w') as file:
    file.write(
        'clean_test_accuracy = '
...
plt.grid()
plt.savefig('CatDogCleanReaderLosses.png')
plt.show()
