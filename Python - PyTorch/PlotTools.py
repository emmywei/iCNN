...
def DisplaySamples2(array1, array2, n=5, file_name=''):
    array1 = to_numpy(array1)
    array2 = to_numpy(array2)
...
def DisplayShiftVariance(test_model, test_data, noise_data,
    ...
    noise_data = to_numpy(noise_data)
    predictions_zero = to_numpy(predictions_zero)
    predictions_shift = to_numpy(predictions_shift)

    if isinstance(iBadRSV, torch.Tensor):
        iBadRSV = int(iBadRSV.detach().cpu().item())
    else:
        iBadRSV = int(iBadRSV)
    ...
    test_shift = np.roll(test_zero, shift, axis=1)
    noisy_shift = np.roll(noisy_zero, shift, axis=1)
    ...
    decoded_back = np.roll(decoded_shift, -shift, axis=1)
...
def PlotShiftVariance(test_model, test_result, y_label):
    test_result = to_numpy(test_result)
